...
def _entity_type(registry: Registry, name: str) -> str:
    canonical = _resolve_alias(registry, name)
    if any(item.name == canonical for item in registry.factions):
        return "faction"
    if any(item.name == canonical for item in registry.characters):
        return "character"
    if any(item.name == canonical for item in registry.locations):
        return "location"
    if any(item.name == canonical for item in registry.events):
        return "event"
    if any(item.label == canonical for item in registry.timeline_anchors):
...
def retrieve_graph(query: str, config: GraphConfig | None = None) -> dict[str, Any]:
    ...
    if matched_entities:
        ...
        if "阵营" in query:
            faction_entity = next(
                (entity for entity in matched_entities if _entity_type(registry, entity) == "faction"),
                first,
            )
            faction_info = query_faction_context(registry, faction_entity)
